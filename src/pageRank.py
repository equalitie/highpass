...
class PageRank:
    def __init__(self, graph, directed, init_ranks = None):
        self.graph = graph
        self.V = len(self.graph)
        self.d = 0.85
        self.directed = directed
        self.ranks = dict()
        self.init_ranks = init_ranks
    
    def rank(self):
        for key, node in self.graph.nodes(data=True):
            if self.directed:
                self.ranks[key] = 1/float(self.V)
                if key in  self.init_ranks:
                    self.ranks[key] *=  self.init_ranks[key]
            else:
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        (graph, init_ranks, content) = parse(filename, isDirected)
        p = PageRank(graph, isDirected, init_ranks)
        p.rank()

        sorted_r = sorted(p.ranks.iteritems(), key=operator.itemgetter(1), reverse=True)

        for rank,tup in enumerate(sorted_r):
            print
            if tup[0] in content:
                print "rank: ", rank
                print '{0:30} :{1:10}'.format(str(tup[0]), tup[1])
                print "Tweets:"
                for cur_content in content[tup[0]]:
                    print cur_content
